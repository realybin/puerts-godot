...
function generatePropertyHelperCode(rule: PropertyRule): { getterName: string; setterName: string; code: string } {
	...
	const code = [
		...
		"\t}",
		"",
		`\tpuerts::internal::bound_instance<godot::${rule.class}> instance = puerts::internal::resolve_bound_instance<godot::${rule.class}>(apis, info, context);`,
		"\tif (!instance.is_valid()) {",
		"\t\treturn;",
		"\t}",
		"",
		`\tpuerts::internal::return_converter<${typeName}>::write(apis, info, context.env, context.environment, ${getterExpr});`,
		"}",
		"",
		...
		"\t}",
		"",
		`\tauto value = puerts::internal::convert_invocation_argument<false, ${typeName}>(apis, info, context, 0);`,
		"\tif (!value.has_value()) {",
		"\t\treturn;",
		"\t}",
		"",
