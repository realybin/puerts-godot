...
uint64_t type_id_key(const void *p_type_id) {
	return reinterpret_cast<uintptr_t>(p_type_id);
}
...
bool should_skip_reflected_property(const Dictionary &p_property_dict) {
	if (!p_property_dict.has("name")) {
		return true;
	}

	const uint32_t usage = p_property_dict.has("usage") ? static_cast<uint32_t>(p_property_dict["usage"]) : PROPERTY_USAGE_DEFAULT;
...
Object *instantiate_reflected_object(const TypeInfo *p_type_info, String &r_error) {
	if (p_type_info == nullptr || p_type_info->kind != TypeInfo::Kind::OBJECT_CLASS) {
		r_error = "Type metadata is missing.";
		return nullptr;
	}
	if (!p_type_info->can_instantiate) {
		r_error = "Type is not instantiable: " + String(p_type_info->class_name);
		return nullptr;
	}
...
void fill_runtime_arguments(
		LocalVector<Variant, int32_t> &r_values,
		LocalVector<const Variant *, int32_t, true> &r_arg_ptrs,
		puerts::internal::callback_context &p_context,
		int p_offset = 0) {
	const int argc = p_context.arg_count;
	const int total = argc + p_offset;
	r_values.resize(total);
	r_arg_ptrs.resize(total);
	...
	for (int i = 0; i < argc; ++i) {
		r_values[i + p_offset] = puerts::script_to_variant(
				...
				p_context.env,
				p_context.get_arg(i));
		r_arg_ptrs[i + p_offset] = &r_values[i + p_offset];
	}
}
...
Object *resolve_holder_object(puerts::internal::callback_context &p_context) {
	...
	if (holder_type != nullptr && holder_type->kind == TypeInfo::Kind::STATIC_BOUND &&
		...
		return static_cast<Object *>(holder);
	}
...
public:
	...
	static TypeInfo *build_static_type(const puerts::StaticTypeDefinition &p_definition) {
		...
		type_info->can_instantiate = p_definition.constructor != nullptr;
...
private:
	static void append_reflected_methods(TypeInfo *p_type_info, const TypedArray<Dictionary> &p_method_list) {
		...
		puerts_eastl::hash_set<StringName, PuertsStringNameHash, PuertsStringNameEqual> seen_instance_methods;
		puerts_eastl::hash_set<StringName, PuertsStringNameHash, PuertsStringNameEqual> seen_static_methods;
		seen_instance_methods.reserve(static_cast<size_t>(p_method_list.size()));
		seen_static_methods.reserve(static_cast<size_t>(p_method_list.size()));

		for (const auto &i : p_method_list) {
			const Dictionary method_dict = i;
			if (!method_dict.has("name")) {
				continue;
			}
			...
			const bool is_static = method_dict.has("flags") && (static_cast<uint32_t>(method_dict["flags"]) & METHOD_FLAG_STATIC) != 0;
			...
			auto &seen_methods = is_static ? seen_static_methods : seen_instance_methods;
	...
	static void append_reflected_signals(TypeInfo *p_type_info, const StringName &p_name) {
		...
		const TypedArray<Dictionary> signal_list = class_db->class_get_signal_list(p_name, true);
		puerts_eastl::hash_set<StringName, PuertsStringNameHash, PuertsStringNameEqual> existing_property_names;
		existing_property_names.reserve(static_cast<size_t>(p_type_info->instance_properties.size() + signal_list.size()));
		for (const TypeInfo::PropertyData &property : p_type_info->instance_properties) {
			existing_property_names.insert(property.name);
		}

		for (const auto &entry : signal_list) {
			const Dictionary signal_dict = entry;
			if (!signal_dict.has("name")) {
				continue;
			}

			TypeInfo::PropertyData signal_property;
			signal_property.name = signal_dict["name"];
			if (!existing_property_names.insert(signal_property.name).second) {
				continue;
			}
			signal_property.variant_type = Variant::OBJECT;
			signal_property.class_name = StringName("Signal");
			signal_property.getter = &PuertsTypeRegister::object_signal_getter_callback;
			signal_property.setter = &PuertsTypeRegister::read_only_property_setter_callback;
	...
	static void append_reflected_enum_groups(PuertsTypeRegister &p_registry, TypeInfo *p_type_info, const StringName &p_name) {
		...
		const PackedStringArray enum_names = class_db->class_get_enum_list(p_name, true);
		...
		for (const auto &enum_name_value : enum_names) {
			...
			enum_type->kind = TypeInfo::Kind::ENUM_GROUP;
			enum_type->class_name = StringName(String(p_name) + "." + String(enum_name));
			enum_type->constructor = nullptr;
			enum_type->finalize = nullptr;
			enum_type->can_instantiate = false;

			const PackedStringArray enum_constants = class_db->class_get_enum_constants(p_name, enum_name, true);
			...
			for (const auto &enum_constant_value : enum_constants) {
				...
				constant.int_constant = class_db->class_get_integer_constant(p_name, enum_constant_name);
				constant.getter = &PuertsTypeRegister::integer_constant_getter_callback;
				constant.setter = &PuertsTypeRegister::read_only_property_setter_callback;
			...
			enum_group_property.setter = &PuertsTypeRegister::read_only_property_setter_callback;
	...
	static void append_reflected_integer_constants(TypeInfo *p_type_info, const StringName &p_name, const PackedStringArray &p_integer_constants) {
		...
		for (const auto &integer_constant : p_integer_constants) {
			...
			const StringName enum_name = class_db->class_get_integer_constant_enum(p_name, constant_name, true);
			if (enum_name != StringName()) {
				continue;
			}
			...
			constant.int_constant = class_db->class_get_integer_constant(p_name, constant_name);
			constant.getter = &PuertsTypeRegister::integer_constant_getter_callback;
			constant.setter = &PuertsTypeRegister::read_only_property_setter_callback;
...
public:
	static void bind(const pesapi_registry_api &p_reg_api, pesapi_registry p_registry, TypeInfo *p_type_info) {
		CharString class_name = String(p_type_info->class_name).utf8();
		p_reg_api.define_class(
				p_registry,
				p_type_info->type_id,
				p_type_info->base_type != nullptr ? p_type_info->base_type->type_id : nullptr,
				"godot",
				class_name.get_data(),
				resolve_constructor(p_type_info),
				p_type_info->finalize,
				p_type_info,
				true);

		p_reg_api.set_property_info_size(
				p_registry,
				p_type_info->type_id,
		...
		bind_methods(p_reg_api, p_registry, p_type_info, p_type_info->instance_methods, false);
		bind_methods(p_reg_api, p_registry, p_type_info, p_type_info->static_methods, true);
		bind_properties(p_reg_api, p_registry, p_type_info, p_type_info->instance_properties, false);
		bind_properties(p_reg_api, p_registry, p_type_info, p_type_info->static_properties, true);
	}

private:
	static pesapi_constructor resolve_constructor(TypeInfo *p_type_info) {
		if (p_type_info->constructor != nullptr) {
			return p_type_info->constructor;
		}
		if (p_type_info->kind == TypeInfo::Kind::OBJECT_CLASS && p_type_info->can_instantiate) {
			return &PuertsTypeRegister::object_default_constructor_callback;
		}
		return &PuertsTypeRegister::no_constructor_callback;
	}

	static void bind_methods(
			const pesapi_registry_api &p_reg_api,
			pesapi_registry p_registry,
			TypeInfo *p_type_info,
			puerts_eastl::vector<TypeInfo::MethodData> &p_methods,
			bool p_is_static) {
		for (int i = 0; i < static_cast<int>(p_methods.size()); ++i) {
			auto *method = &p_methods[i];
			CharString method_name = String(method->name).utf8();
			p_reg_api.set_method_info(
					p_registry,
					p_type_info->type_id,
					i,
					...
					p_is_static,
					method->callback,
					method->userdata != nullptr ? method->userdata : method,
					true);
		}
	}
	...
	static void bind_properties(
			...
			pesapi_registry p_registry,
			TypeInfo *p_type_info,
			puerts_eastl::vector<TypeInfo::PropertyData> &p_properties,
			bool p_is_static) {
		for (int i = 0; i < static_cast<int>(p_properties.size()); ++i) {
			...
			p_reg_api.set_property_info(
					p_registry,
					p_type_info->type_id,
					i,
					property_name.get_data(),
					p_is_static,
					property->getter,
					...
					property->setter != nullptr ? (property->setter_userdata != nullptr ? property->setter_userdata : property) : property,
					true);
		}
	}
};
...
PuertsTypeRegister::PuertsTypeRegister() {
	...
	on_type_not_found_ = [](const void *type_id) -> int {
		TypeInfo *type_info = PuertsTypeRegister::get_singleton().get_type_by_id(type_id);
		return type_info != nullptr && PuertsTypeRegister::get_singleton().ensure_registered(type_info);
	};
	reg_api_.on_class_not_found(registry_, on_type_not_found_);
}
...
TypeInfo *PuertsTypeRegister::get_type_by_id(const void *p_type_id) {
	auto found = types_by_id_.find(type_id_key(p_type_id));
	return found == types_by_id_.end() ? nullptr : found->second;
}

TypeInfo *PuertsTypeRegister::get_builtin_type(Variant::Type p_type) {
	const int type_index = p_type;
	if (type_index < 0 || type_index >= Variant::VARIANT_MAX) {
		return nullptr;
	}
	return builtin_types_by_variant_[type_index];
}

PuertsTypeRegister::TypeInfo *PuertsTypeRegister::build_object_type(const StringName &p_name) {
	return puerts_type_register_internal::TypeInfoFactory::build_object_type(*this, p_name);
}
...
TypeInfo *PuertsTypeRegister::find_or_add_object_type(const StringName &p_name) {
	...
	TypeInfo *type_info = build_object_type(p_name);
...
void PuertsTypeRegister::store_type(TypeInfo *p_type_info) {
	...
	types_by_id_[type_id_key(p_type_info->type_id)] = p_type_info;
...
void PuertsTypeRegister::register_type(TypeInfo *p_type_info) {
	puerts_type_register_internal::RegistryBinder::bind(reg_api_, registry_, p_type_info);
...
void *PuertsTypeRegister::object_default_constructor_callback(struct pesapi_ffi *apis, pesapi_callback_info info) {
	...
	if (!puerts::internal::resolve_context(apis, info, context, false, true)) {
		return nullptr;
	}
...
void PuertsTypeRegister::object_method_callback(struct pesapi_ffi *apis, pesapi_callback_info info) {
	...
	puerts::internal::callback_context context;
	if (!puerts::internal::resolve_context(apis, info, context, true, true)) {
		return;
	}
...
void PuertsTypeRegister::object_static_method_callback(struct pesapi_ffi *apis, pesapi_callback_info info) {
	auto *method = static_cast<TypeInfo::MethodData *>(apis->get_userdata(info));
	puerts::internal::callback_context context;
	if (!puerts::internal::resolve_context(apis, info, context, true, false)) {
		return;
	}
...
void PuertsTypeRegister::object_property_setter_callback(struct pesapi_ffi *apis, pesapi_callback_info info) {
	...
	if (context.arg_count < 1) {
		apis->throw_by_string(info, "Missing property value.");
		return;
	}
...
void PuertsTypeRegister::object_signal_getter_callback(struct pesapi_ffi *apis, pesapi_callback_info info) {
	...
	if (!object->has_signal(property->name)) {
		CharString message = String("Signal not found: " + String(property->name)).utf8();
		apis->throw_by_string(info, message.get_data());
		return;
	}
...
void PuertsTypeRegister::enum_group_getter_callback(struct pesapi_ffi *apis, pesapi_callback_info info) {
	...
	if (enum_type == nullptr) {
		apis->throw_by_string(info, "Enum type metadata is missing.");
		return;
	}
...
void PuertsTypeRegister::load_type_callback(struct pesapi_ffi *apis, pesapi_callback_info info) {
	...
	auto *type_info = PuertsTypeRegister::get_singleton().find_type_by_name(class_name);
	if (type_info == nullptr || !PuertsTypeRegister::get_singleton().ensure_registered(type_info)) {
